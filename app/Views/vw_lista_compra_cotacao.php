...
<div id='content' class='container page-content m-0'>
  <div class='col-12 d-block p-3 float-start bg-white' style="height: 90vh">
    ...
    if (isset($destino)) { ?>
      ...
      <div id="divTable" class="col-12 px-2 overflow-auto" style="max-height: 60vh !important;overflow:auto">
        <table id="tb_<?= $nome; ?>" class="table table-sm table-success table-hover table-borderless col-12">
          <thead class="table-success col-12 sticky-top">
            <tr>
              <?
          ...
          <div id="linhaProdutoFixada" style="display: none; position: fixed; top: 78px;width:94%;z-index: 9999; background-color: #f2f2f2; border-bottom: 2px solid #ccc;"></div>
          <tbody id="itens_tabela" class="text-center mt-3">
          </tbody>
        </table>
      </div>
  ...
  <script>
    let $fixo = jQuery('#linhaProdutoFixada');

    jQuery(document).ready(function() {
      let linhaAtual = null;
      $fixo.hide();

      jQuery("#divTable").on('scroll', function() {
        const scrollTop = jQuery(this).scrollTop();
        const containerTop = jQuery("#divTable").offset().top + 80;

        let novaLinha = null;

        // Percorre todas as linhas de produto
        jQuery('.linha_produto').each(function(index) {
          const $this = jQuery(this);
          const offsetTop = $this.offset().top;

          // Quando a linha alcança o topo da divTable
          if (offsetTop <= containerTop) {
            novaLinha = $this;
          }
        });

        // Se nova linha detectada e diferente da atual
        if (novaLinha && (!linhaAtual || !novaLinha.is(linhaAtual))) {
          linhaAtual = novaLinha;
          const clone = linhaAtual.clone();
          const tabelaClone = jQuery('<table class="table table-bordered mb-0"></table>').append(clone);
          $fixo.html(tabelaClone).show();
        }

        // Se nenhuma linha está acima do topo, esconde
        if (!novaLinha) {
          $fixo.hide();
          linhaAtual = null;
        }
