 ...
 * @return array
 */
function detalhes_classe(string $nomeClasse, array $retorno){
    preg_match('/\\\\([\w]+)$/', $nomeClasse, $nome);
    $perfil_usu             = session()->get('usu_perfil_id');
    $classe                 = new \App\Models\AdmClasseModel();
    $perfil_pit             = new \App\Models\AdmPerfilItemModel();
    $menu                   = new \App\Models\AdmMenuModel();
    $busca_classe           = $classe->getClasse($nome[1])[0];
    $retorno['title']       = $busca_classe['clas_icone'].' '.$busca_classe['clas_titulo'];
 ...
 * @return string
 */
function ver_codigo($nomeClasse){
...
function debug($valor, $parada = true)
{
    if (is_array($valor) || is_object($valor)) {
        echo "array";
        echo "<div style = 'border:1px solid green; background-color: #ccc; color:green;width:100%; z-index:5000'>";
        echo "<pre>";
        print_r($valor);
        echo "</pre>";
        echo "</div>";
