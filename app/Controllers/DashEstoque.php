	...
	public function busca_dados()
	{
        ...
        for($s=0;$s<count($saldos);$s++){
            ...
            if($prod['qtia_ultima_contagem'] + $prod['total_entradas'] + $prod['total_saidas'] > 0 ){
                ...
                $prods[$ct][3] = formataQuantia($prod['qtia_ultima_contagem'],3)['qtia'];
                $prods[$ct][4] = formataQuantia($prod['total_entradas'],3)['qtia'];
                $prods[$ct][5] = formataQuantia($prod['total_saidas'],3)['qtia'];
                $prods[$ct][6] = formataQuantia($prod['saldo'],3)['qtia'];
