...
class EstSaida extends BaseController {
    ...
    */
    ...
    //         $campos = montaColunasCampos($this->data, 'sai_id');
    //         $dados_entrad = $this->saida->getSaidaLista();
    //         $entrad = [
    //             'data' => montaListaColunas($this->data, 'sai_id', $dados_entrad, $campos[1]),
    //         ];
    ...
    */
    public function lista_prod()
    {
        ...
        // if (!$compr = cache('compr')) {
            ...
            $saida = montaListaColunas($this->data, 'sai_id', $dados_saida, $campos[1], true);
            for ($cp=0; $cp < count($saida) ; $cp++) { 
                $said =$saida[$cp];
                $saida[$cp]['col_details'] =[
                    'tit' => ['Produto','Qtia','Fator Conv','Qtia Conv','Und','Destino','Data'],
                    'tam' => ['col-4','col-1','col-1','col-1','col-1','col-1','col-2'],
                    'cam' => ['pro_nome','sap_quantia','sap_conversao','sap_qtia_conv','und_sigla','sap_destino','sai_datahora'],
                ];
                $dados_prods = $this->saida->getSaidaProd($said[0]);
                for ($p=0; $p < count($dados_prods) ; $p++) { 
                    $qtia = formataQuantia(isset($dados_prods[$p]['sap_quantia'])?$dados_prods[$p]['sap_quantia']:0);
                    $dados_prods[$p]['sap_quantia'] = $qtia['qtia'];
                    $conv = formataQuantia(isset($dados_prods[$p]['sap_conversao'])?$dados_prods[$p]['sap_conversao']:0);
                    $dados_prods[$p]['sap_conversao'] = $conv['qtia'];
                    $qcon = formataQuantia(isset($dados_prods[$p]['sap_qtia_conv'])?$dados_prods[$p]['sap_qtia_conv']:0);
                    $dados_prods[$p]['sap_qtia_conv'] = $qcon['qtia'];
                    $dados_prods[$p]['sai_datahora'] = dataDbToBr($dados_prods[$p]['sai_datahora']);
                    
                    $saida[$cp]['details'][$p] = $dados_prods[$p];
                }
            }
            $saidas['data'] = $saida;
