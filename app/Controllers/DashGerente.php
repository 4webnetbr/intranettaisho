        ...
        if($busca == 'fat_dia'){
            ...
            if($busca == 'nps_dia'){
                ...
                for($n=0;$n<count($nps_res); $n++){
                    ...
                    // }
                    $diaatual = date('Y-m-d',strtotime("+1 days",strtotime($dia)));
                }
                // krsort($notas);
                // krsort($somas);
                // debug($notas);
                foreach($notas as $dia => $val){
                    ...
                    $npsmedio[$dia]     = 0;
                    if($total[$dia] >0){
                        $promotopct[$dia] = number_format(($promotores[$dia] * 100) / $total[$dia],0);
                        $neutrospct[$dia] = number_format(($neutros[$dia] * 100) / $total[$dia],0);
                        $detratopct[$dia] = number_format(($detratores[$dia] * 100) / $total[$dia],0);
                        $npsmedio[$dia] = number_format((($promotores[$dia] - $detratores[$dia]) * 100) / $total[$dia],0);
                    }
                }
                if(isset($npsmedio)){
                    $ct = 0;
                ...
                foreach($notas as $mesano => $val){
                    ...
                    $promotopct[$mesano] = 0;
                    $promotopct[$mesano] = number_format(($promotores[$mesano] * 100) / $total[$mesano],0);
                    $neutrospct[$mesano]    = 0;
                    $neutrospct[$mesano] = number_format(($neutros[$mesano] * 100) / $total[$mesano],0);
                    $detratopct[$mesano] = 0;
                    $detratopct[$mesano] = number_format(($detratores[$mesano] * 100) / $total[$mesano],0);

                    $npsmedio[$mesano] = number_format((($promotores[$mesano] - $detratores[$mesano]) * 100) / $total[$mesano],0);
                }
                if(isset($npsmedio)){
                    $ct = 0;
                ...
                if($nps_res){
                    ...
                    $ret['dados'][0]['Valor'] = number_format($promotopct,0);
                    $cores[0] = $cornota[10];

                    $ret['dados'][1]['Nota'] = 'Neutros';
                    $ret['dados'][1]['Valor'] = number_format($neutpct,0);
                    $cores[1] = $cornota[8];

                    $ret['dados'][2]['Nota'] = 'Detratores';
                    $ret['dados'][2]['Valor'] = number_format($detratopct,0);
                    $cores[2] = $cornota[0];

                    $ret['cores'] = $cores;
                    $ret['nps'] = number_format(($promotopct - $detratopct),0);
                    $ret['respostas'] = $respostas;
                }
            }
