function montaGrafico(idGraf, valores, tipo, cores) {
    ...
    if (tipo == 'p_') {
                            ...
                            label: function (context) {
                                ...
                                if (context.parsed !== null) {
                                    if (context.parsed < 100) {
                                        ...
                                        label += converteFloatMoeda(context.parsed).slice(0, -3).slice(3);;
        ...
        let maxDataValue = Math.round(Math.max.apply(null, lineChartData.datasets[1].data));
                            ...
                            label: function (context) {
                                ...
                                if (context.parsed.y !== null) {
                                    if (context.parsed.y < 100) {
                                        label += new Intl.NumberFormat('pt-BR', { style: 'decimal' }).format(context.parsed.y) + '%';
                                    } else {
                                        label += converteFloatMoeda(context.parsed.y).slice(0, -3).slice(3);
                    ...
                    datalabels: {
                        ...
                        formatter: function (value, context) {
                            if (value < 100) {
                                ...
                                return converteFloatMoeda(parseInt(value)).slice(0, -3).slice(3);
                                // return parseInt(Math.round(value));
                            }
                        },
                        borderRadius: 4,
                            ...
                            label: function (context) {
                                ...
                                if (context.parsed.y !== null && !isNaN(context.parsed.y)) {
                                    if (context.parsed.y < 100) {
                                        ...
                                        label += converteFloatMoeda(parseInt(context.parsed.y)).slice(0, -3).slice(3);
                                    }
                                }
                                return label;
                            }
...
function criaViewCard(nome, titulo, colunas, agrupa, graficos, dest, ccol = 'col-6') {
    ...
    txt += '<i id="j_' + nome + '" class="fa-solid fa-arrow-up-right-from-square btn px-1 text-info" onclick="graf_janela(' + nome + ')"></i>';
    ...
    txt += '<i id="t_' + nome + '" class="fa-solid fa-table-cells btn px-1 text-info" onclick="tabe_janela(' + nome + ')"></i>';
    txt += '</div>';
...
function graf_janela(obj) {
    ...
    jQuery.post("/DashGerente/secao", { "colunas": colunas, "nome": nome, "tipo": tipo, "inicio": startDate, "fim": endDate, "empresa": empresa, "busca": nome, "titulo": titulo });
...
function refresh_card(obj, grafico) {
    ...
    jQuery.ajax({
        ...
        async: true,
        dataType: 'json',
        data: { 'busca': busca, 'periodo': periodo, 'inicio': startDate, 'fim': endDate, 'empresa': empresa, 'tipo': grafico },
        beforeSend: function () {
        ...
        success: async function (retorno) {
            if (retorno.registros > 0) {
                ...
                if (grafico) {
                    ...
                    if (busca == 'nps_notas') {
                        jQuery('#nps_notas')[0].children[0].innerHTML = 'NPS - Notas Recebidas - Nota Média ';
                    }
                    if (busca == 'nps_clientes') {
                        jQuery('#nps_clientes')[0].children[0].innerHTML = 'NPS - ' + retorno.nps + ' - Respostas: ' + retorno.respostas;
                    }
                    if (busca == 'nps_dia') {
                        jQuery('#nps_dia')[0].children[0].innerHTML = 'NPS médio por dia - Respostas: ' + retorno.respostas;
                    }
            ...
            if (retorno.registros > 0) {
                ...
                if (grafico) {
                    ...
                    if (busca.substr(0, 7) == 'nps_not') {
                        jQuery('#' + busca)[0].children[0].innerHTML = jQuery('#' + busca)[0].children[0].innerHTML + 'NPS - Notas Recebidas - Nota Média ';
                    }
                    if (busca.substr(0, 7) == 'nps_cli') {
                        jQuery('#' + busca)[0].children[0].innerHTML = jQuery('#' + busca)[0].children[0].innerHTML + ' - NPS - ' + retorno.nps + ' - Respostas: ' + retorno.respostas;
                    }
                    if (busca.substr(0, 7) == 'nps_dia') {
                        jQuery('#' + busca)[0].children[0].innerHTML = jQuery('#' + busca)[0].children[0].innerHTML + ' - Respostas: ' + retorno.respostas;
                    }
    ...
    for (r = 0; r < arrResult.length; r++) {
        ...
        for (c = 0; c < numColuns; c++) {
            ...
            if (ehnumero) {
                ...
                if (parseFloat(arrCols[c][1]) > 100 && !Number.isInteger(parseFloat(arrCols[c][1]))) {
                ...
                dtsort = "data-sort = '" + moment(datafmt, 'DD/MM/YYYY').format('YYYY-MM-DD') + fim + "'";
            }
            linha = linha + "<td " + dtsort + " class='" + classe + "'>" + arrCols[c][1] + "</td>";
            if (c > 0) {
...
function addGrupo(idTabela, arrResult) {
    ...
    for (r = 0; r < arrResult.length; r++) {
        ...
        if ($grupo != arrCols[0][1]) {
            ...
            linha = linha + "<td colspan='" + numColuns + "'><h4>" + arrCols[0][1] + "</h4></td></tr><tr>";
            $cc = 1;
            for (c = 1; c < numColuns; c++) {
                subtotal[c] = 0;
            }
        ...
        for (c = 0; c < numColuns; c++) {
            ...
            linha = linha + "<td class='" + classe + "'>" + arrCols[c + $cc][1] + "</td>";
    ...
    for (r = 0; r < arrResult.length; r++) {
        ...
        if ($subgrupo != arrCols[1][1] || $grupo != arrCols[0][1]) {
            ...
            if ($grupo != arrCols[0][1]) {
                ...
                linha = linha + "<td colspan='" + numColuns + "'><h4 class='bg-success-opacity text-start' >" + arrCols[0][1] + "</h4></td></tr><tr>";
                $cc = 2;
                for (c = 1; c < numColuns; c++) {
                    subtotal[c] = 0;
                }
            ...
            linha = linha + "<td colspan='" + numColuns + "'><h4>" + arrCols[1][1] + "</h4></td></tr><tr>";
            $cc = 2;
            for (c = 1; c < numColuns; c++) {
                subsubtotal[c] = 0;
            }
        ...
        for (c = 0; c < numColuns; c++) {
            ...
            if (c > 0) {
                ...
                if ($subgrupo != '') {
                    subsubtotal[c] = subsubtotal[c] + parseInt(Math.round(arrCols[c + $cc][1]));
                }
                total[c] = total[c] + parseInt(Math.round(arrCols[c + $cc][1]));
            }
        }
