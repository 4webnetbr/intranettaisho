        ...
        tooltip: {
          callbacks: {
            label(context) {
              ...
              return [
                `Almoço: ${converteFloatMoeda(almoco)} (${pAlmoco.toFixed(
                  1
                )}%)`,
...
function criaViewCard(
  ...
  txt +=
    '<i id="j_' +
    nome +
    '" class="fa-solid fa-arrow-up-right-from-square btn px-1 text-info" onclick="abrirCardEmNovaGuia(this)"></i>';
  ...
  txt +=
    '<i id="t_' +
    nome +
    '" class="fa-solid fa-table-cells btn px-1 text-info" onclick="abrirTabelaEmNovaGuia(this)"></i>';
  txt += "</div>";
  txt +=
...
function addGrupo(idTabela, arrResult) {
  ...
  for (var r = 0; r < arrResult.length; r++) {
    ...
    if (current != arrCols[0][1]) {
      ...
      current = arrCols[0][1];
      jQuery("#" + idTabela + " tbody").append(
        "<tr><td colspan='" + numColuns + "'><h4>" + current + "</h4></td></tr>"
      );
    }
    var linha = "<tr>";
    for (var c = 0; c < numColuns; c++) {
      ...
      linha +=
        "<td class='" +
        (jQuery.isNumeric(val) ? "text-end" : "text-start") +
        "'>" +
        val +
        "</td>";
      if (c > 0) {
