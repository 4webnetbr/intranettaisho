 ...
 * @param {*} total
 */
function calcula_tot_it_compra(
  orig,
  quantia,
  unitario,
  frete,
  ...
  vunit = converteMoedaFloat(
    jQuery("#" + unitario + "\\[" + indice + "\\]").val()
  );
...
function calcula_tot_impostos(orig, imposto, total) {
  ...
  jQuery("[id^='" + imposto + "']").each(function () {
    if (imposto == "mpc_ipi") {
      vunit = converteMoedaFloat(
        jQuery("#mpc_unitario\\[" + indice + "\\]").val()
      );
      pctipi = this.value;
      vaipi = vunit * (pctipi / 100);
      valor = vaipi;
...
function rateia_frete_mat() {
  ...
  if (total_frete > 0) {
    ...
    jQuery("[id^='mpc_frete']").each(function () {
      ...
      tot_mat = converteMoedaFloat(
        jQuery("#mpc_total\\[" + indice + "\\]").val()
      );
      pct_mat = (tot_mat * 100) / total_compra;
      fre_mat = total_frete * (pct_mat / 100);
      jQuery(this).val(converteFloatMoeda(fre_mat));
      des_mat = converteMoedaFloat(
        jQuery("#mpc_desconto\\[" + indice + "\\]").val()
      );
...
function rateia_desconto_mat() {
  ...
  jQuery("[id^='mpc_desconto']").each(function () {
    ...
    if (val_des_mat > 0 && jQuery(this).attr("data-alter")) {
      ...
      // $ind     = jQuery(this).attr('data-index');
      indice = this.getAttribute("data-index");
      fre_mat = converteMoedaFloat(
        jQuery("#mpc_frete\\[" + indice + "\\]").val()
      );
      tot_mat = converteMoedaFloat(
        jQuery("#mpc_total\\[" + indice + "\\]").val()
      );
      ...
      jQuery("#mpc_pct_desconto\\[" + indice + "\\]").val(
        parseFloat(pct).toFixed(3)
      ); // pct_desconto é decimal não é moeda
...
function calcula_desconto(orig, pct_desconto, val_desconto) {
  ...
  uni_mat = converteMoedaFloat(
    jQuery("#mpc_unitario\\[" + indice + "\\]").val()
  );
  pct = jQuery("#" + pct_desconto + "\\[" + indice + "\\]").val();
  val = converteMoedaFloat(
    jQuery("#" + val_desconto + "\\[" + indice + "\\]").val()
  );
  if (
    campo == "mpc_pct_desconto" &&
    jQuery("#mpc_pct_desconto\\[" + indice + "\\]").attr("data-alter")
  ) {
    val = uni_mat * (pct / 100);
