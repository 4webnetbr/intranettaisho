...
function carrega_pedidos(obj) {
    ...
    if (empresa != '') {
        // jQuery('#' + tabela).closest('tr').remove();
        url = "/EstRelPedido/busca_dados?empresa=" + empresa + "&inicio=" + startDate + "&fim=" + endDate;
        montaListaDadosGrupo(tabela, url, 1);
    }
}
...
function carrega_movimentos(obj) {
    ...
    if (empresa != '') {
        // jQuery('#' + tabela).closest('tr').remove();
        url = "/EstRelMovimento/busca_dados?empresa=" + empresa + "&inicio=" + startDate + "&fim=" + endDate;
        ...
        jQuery.ajax({
            ...
            success: function (retorno) {
                ...
                retorno.data.forEach(function (item) {
                    ...
                    // monta o card
                    var card = jQuery('<div>', { class: 'card mb-2 w-100' });
                    ...
                    jQuery.each(campos, function (label, valor) {
                        var col = jQuery('<div>', { class: 'col-12 col-lg-1 mb-2' });
                        if(label == 'Produto' || label == 'Marca' || label == 'Apresentação' || label == 'Und'){
                            var col = jQuery('<div>', { class: 'col-12 col-lg-2 mb-2' });
                        }
                        col.html(
                            '<strong>' + label + ':</strong><br>' +
                            '<span class="text-start">'+valor+'</span>'
                        );
                        row.append(col);
                    });
